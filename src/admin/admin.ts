import { operations } from "../../types/admin";
...
class Admin {
  ...
  async postBoostProfile(coachId: string) {
    if (await this._client.checkAuth())
  ...
  async getCategoryImport(formData: FormData) {
    if (await this._client.checkAuth())
      return await this._client.get(`/admin/category/import`, formData, null);
  }

  // Business endpoints
  async getBusinessImport(formData: FormData) {
    if (await this._client.checkAuth())
      return await this._client.get(`/admin/category/import`, formData, null);
  }
  ...
  async getRefundDetails(reportId: string) {
    if (await this._client.checkAuth())
