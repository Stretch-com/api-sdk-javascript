...
export class StretchBase{
    ...
    _updateToken(res){
        ...
        if (res.hasOwnProperty("access_token")){
            ...
            if (storageExists) localStorage.setItem("access_token", res["access_token"]);
        }
        if (res.hasOwnProperty("refresh_token")){
            this.#refreshToken = res["refresh_token"]
            if (storageExists) localStorage.setItem("refresh_token", res["refresh_token"]);
        }
        ...
        if (res.hasOwnProperty("access_expire")){
            ...
            if (storageExists) localStorage.setItem("access_expire_date", currentTime);
        }

        if (res.hasOwnProperty("refresh_expire")){
            let currentTime = new Date();
            currentTime.setSeconds(currentTime.getSeconds() + res["refresh_expire"] - 60);
            this.#refreshExpireDate = currentTime
            if (storageExists)  localStorage.setItem("refresh_expire_date", currentTime);
        }
        return res;
    }
    ...
    isLogin(){
        try{
            if (this.#accessToken == null) {
                ...
                this.#accessExpireDate = new Date(Date.parse(localStorage.getItem("access_expire_date")));
            }
            if (this.#refreshToken == null) {
                this.#refreshToken = localStorage.getItem('refresh_token');
                this.#refreshExpireDate = new Date(Date.parse(localStorage.getItem("refresh_expire_date")));
            }
       }catch (e){
            console.error(`LocalStorage in current context not exists ${e}`);
        }
        ...
        if (this.isLogin()) {
            if (this.#accessExpireDate instanceof Date) {
                ...
                if (current_date.getTime() > this.#accessExpireDate.getTime()) {
                    if (this.#refreshToken != null && current_date.getTime() < this.#refreshExpireDate.getTime()) {
                        await this.refresh()
                    } else {
